# Versioning — ask-channel-providers
...
- **PATCH** (`0.2.0` → `0.2.1`): bug fixes, internal refactors, docs, and
  tests. No intentional behavior or public API change.
- **MINOR** (`0.2.0` → `0.3.0`): new features, new public API, and any
...
Patch numbers advance one step at a time: `0.2.0` → `0.2.1` → `0.2.2`.
Never jump patch numbers (no `0.2.0` → `0.2.3`), even when several fixes
...
- Every `ask-*` gem (including ask-channel-providers) and **yamine** is
  released with `gemchain` from the workspace root — never `rake release`,
...
- **Dependency releases use the gemchain cascade**: when a dependency of
  ask-channel-providers (or a gem that depends on it) changes, run
  `gemchain guard` to see the blast radius and `gemchain update` to bump,
  test, and release every affected gem in topological order. Never release
  the dependent by hand.
...
2. **Build** — `gem build ask-channel-providers.gemspec` succeeds
   (gemchain does this during release; a manual build is a pre-flight
   sanity check only).
...
Starting version `0.2.0`, all released via
`gemchain update ask-channel-providers <ver>` from the workspace root:

| Change | Next version | Why |
|---|---|---|
| Fix signature verification on webhook payloads | `0.2.1` | pre-1.0 fix ⇒ sequential PATCH |
| Three bug fixes land together | `0.2.2` (one release, not `0.2.3`) | one step, however many fixes |
| Add a new channel adapter | `0.3.0` | new feature ⇒ pre-1.0 MINOR |
| Rename a public class (breaking) | `0.3.0` | breaking ⇒ pre-1.0 MINOR |
| Declare the API stable | `1.0.0` | first stable major |
| A gem ask-channel-providers depends on releases | cascade via `gemchain update <gem> <ver>` | dependents handled by the cascade, never by hand |

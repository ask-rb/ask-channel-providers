...
module Ask
  module ChannelProviders
    ...
    # identifies the sender on the provider.
    class InboundMessage
      ATTRIBUTES = %i[provider external_uid channel_id message_id kind text name timestamp].freeze

      attr_reader(*ATTRIBUTES)

      def initialize(provider:, external_uid:, channel_id:, message_id:, kind: "text", text: nil, name: nil, timestamp: nil)
        @provider = provider
